...
.. http:post::  /v2.4/verify/evidence
    ...
    * Fleet management systems that manage their own trust but want to check attestation evidence like TPM quotes, IMA logs, Measured Boot against some policy
    ...
    **Example request**:
    ...
    :<json string quote: The quote from the TPM
    :<json string hash_alg: The hashing algorithm used by the TPM
    :<json string tpm_ak: AK of the agent, base64-encoded.
    :<json string tpm_ek: EK of the agent, base64-encoded.
    :<json string tpm_policy: Static PCR policy and mask for TPM. Is a string encoded dictionary that also includes a `mask` for which PCRs should be included in a quote. Optional
    :<json string runtime_policy: JSON document of a Keylime Runtime (IMA) policy. Optional
    :<json string mb_policy: JSON document of a Keylime Measured Boot policy. Optional
    :<json string ima_measurement_list: The ASCII content of the IMA measurement list. Usually found at /sys/kernel/security/ima/ascii_runtime_measurements. Optional depending on runtime_policy.
    :<json string mb_list: The binary contents of the boot log. Usually found at something like /sys/kernel/security/tpm0/binary_bios_measurements. Optional depending on mb_policy.
    ...
    **Example response**:
    ...
    :>json string status: Status as string
    :>json object results: Results as a JSON object
    :<json int valid: A boolean 1 for valid, 0 for invalid evidence
